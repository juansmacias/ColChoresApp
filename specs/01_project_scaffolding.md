...
- Isar schema definitions (see `specs/02_isar_schemas.md`)
...
- The Isar maintenance status decision gate has been resolved (Isar is used unless explicitly switched to Drift).
...
|   |   |   +-- modules/
|   |   |       +-- database_module.dart     # Isar instance provider
...
abstract class DatabaseModule {
  ...
  Future<Isar> get isar async {
    ...
    return Isar.open(
      [
        // Schema list registered here -- see specs/02_isar_schemas.md
        TaskEntitySchema,
        MemberEntitySchema,
        FamilyEntitySchema,
        RewardEntitySchema,
...
class TaskRepositoryImpl implements TaskRepository {
  ...
  @override
  Future<Result<List<Task>>> getTasksForMember(String memberId) {
    // Implementation reads from local Isar only
  }
}
```
...
| DI configuration | New | Medium | Must handle async initialization (Isar, SharedPreferences) |
...
|------|------------|--------|------------|
| Isar package incompatibility with latest Flutter | Medium | High | Decision gate: verify Isar compatibility at Phase 1 start. Have Drift migration plan ready. |
...
### 14.3 Isar Decision Gate

Before executing this spec, verify Isar's maintenance status:

1. Check the Isar GitHub repository for recent commits and open issues.
2. Verify compatibility with the latest stable Flutter SDK.
3. Run `flutter pub add isar isar_flutter_libs` and confirm resolution.
4. If Isar is unmaintained or incompatible, switch to Drift:
   - Replace `isar` and `isar_flutter_libs` with `drift` and `sqlite3_flutter_libs`.
   - Replace `isar_generator` with `drift_dev`.
   - Update `DatabaseModule` in DI config.
   - Update all schema specs (02_isar_schemas.md) to use Drift table definitions.
   - The rest of the architecture remains unchanged -- only the local datasource implementation changes.
